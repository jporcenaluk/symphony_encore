...
| S05 | Issue and SystemJob lifecycle | In progress | Lane policy, deterministic eligibility/candidate ordering, a serialized/coalesced periodic/immediate scheduler with production start/stop lifecycle, exact reconciliation-first poll ordering, atomic first-observation/external-lane history, receipt-gated issue launch with synchronized durable issue state, atomic issue and tracker-free SystemJob dispatch/closure, bounded issue/repair outcome routing, and running-attempt reconciliation ordering pass; concrete production port assembly and plan/review routes remain pending |
...
| S13 | Failure classification, retry, and restart recovery | In progress | Failure routing, intent reconstruction, interrupted closure with latest durable or factual first-attempt handoff, verified Linux tree termination, workspace recovery, production recovering-to-ready startup, operator-store corruption shutdown, orderly durable stop, a first-failure persistence safety latch, typed claim reconstruction, CAS lease renewal, retry-delay rebuilding, parked cursor/predicate rehydration, and terminal/lane/eligibility/stall reconciliation pass; live scheduler integration remains pending |
